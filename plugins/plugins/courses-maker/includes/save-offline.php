...
if( current_user_can('publish_posts')) {
  ...
  $guid   = get_site_url('','offline-courses/'.$new_post_name);
  ...
  if (intval($_POST['id'])) {
    ...
    if (!$wpdb->insert($table_name, $values)) {
      ...
      wp_redirect(get_site_url().'/offline-courses&err='.urlencode($wpdb->last_error));
    ...
    $meta_value_1 = $_POST['offline_date'];
    $meta_value_2 = $_POST['offline_date_end'];
    $meta_value_3 = $_POST['location_value'];
    $meta_value_4 = $_POST['location_adress'];
    $meta_value_5 = $_POST['course_free_places'];

    set_post_thumbnail( $post_id, $_POST['attachment_id'] );
    update_post_meta( $post_id, '_offline_date', $meta_value_1);
    update_post_meta( $post_id, '_offline_date_end', $meta_value_2);
    update_post_meta( $post_id, '_location_value', $meta_value_3);
    update_post_meta( $post_id, '_location_adress', $meta_value_4);
    update_post_meta( $post_id, '_course_free_places', $meta_value_5);
