<?php
  ...
  *
  ...
  class pagination
  {
     ...
     * 
     * @param string $mainSeparator The seperator between the page numbers
     * @return void    
     * @access public  
     */
    public function setMainSeparator($mainSeparator)
    {
      ...
      if (($this->pages) > 1) {
        ...
        if ($this->library_page != 1) {
          if ($this->_showFirstAndLast) {
            $plinks[] = ' <a href="?library_page=1'.$queryUrl.'">&laquo;&laquo; ' . __('First','module-manager') . ' </a> ';
          }
          $plinks[] = ' <a href="?library_page='.($this->library_page - 1).$queryUrl.'">&laquo; ' . __('Prev','module-manager') . '</a> ';
        }
        ...
        if ($this->library_page < $this->pages) {
          $slinks[] = ' <a href="?library_page='.($this->library_page + 1).$queryUrl.'"> ' . __('Next','module-manager') . ' &raquo; </a> ';
          if ($this->_showFirstAndLast) {
            $slinks[] = ' <a href="?library_page='.($this->pages).$queryUrl.'"> ' . __('Last','module-manager') . ' &raquo;&raquo; </a> ';
          }
        }
        
        // Push the array into a string using any some glue
        return implode(' ', $plinks).implode($this->mainSeparator, $links).implode(' ', $slinks);
      }
      return;
    }
  }