...
//					});
	...
	jQuery(document).on('click', '.upload_image_button', function( event ){
		...
		file_frame.on( 'select', function() {
			...
			parent_li.find('.wpProQuiz_text').first().val(parent_li.find('.wpProQuiz_text').first().val()+' <img src="'+attachment.url+'" class="size-full" />');
	...
	jQuery(document).on('click', 'input[name="answerType"]', function(){
		if(jQuery(this).val() == 'essay'){
			if(jQuery('input[name="answerPointsActivated"]').attr('checked') != 'undefined'){
				jQuery('input[name="answerPointsActivated"]').trigger('click');
				jQuery('input[name="showPointsInBox"]').removeAttr('checked');
			}
			jQuery('input[name="answerPointsActivated"]').parent().parent().hide();
			jQuery('input[name="showPointsInBox"]').parent().parent().hide();
		}else{
			jQuery('input[name="answerPointsActivated"]').parent().parent().show();
		}
	});
});